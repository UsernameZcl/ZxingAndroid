...
//                                config.setScanLineColor(R.color.colorAccent);//设置扫描线的颜色 默认白色
                                ...
                                config.setContinuity(true);//是否连续扫描
//                                config.setRule(true);//扫描的规则    config.setCodeLength(12)或者  config.setPipelinedCode(true); 必须设置一个
//                                config.setCodeLength(12);//条码的长度
                                config.setTitle("记录仪扫描");
                                config.setScanNumber(3);
                                config.setShowText(true);
                                config.setPipelinedCode(true);//条码是否为纯流水码
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == REQUEST_CODE_SCAN && resultCode == RESULT_OK) {
            if (data != null) {
                ...
                List<String> backLists = (List<String>) data.getSerializableExtra(Constant.CODED_CONTENT);
                result.setText("扫描结果为：" + backLists.toString());
            }
        }
    }
